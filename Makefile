...
# Get the currently used golang install path (in GOPATH mode, this is
# just the value of GOPATH, but in module mode, it's the path to the
# module directory).
...
.PHONY: controller-gen-check
controller-gen-check: ## Check if controller-gen is installed
